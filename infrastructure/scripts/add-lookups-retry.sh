#!/usr/bin/env bash
# FlightDeck — Retry failed lookup relationships (one at a time with long delays)
set -uo pipefail
...
echo "=== Retrying failed lookup relationships ==="
...
# 5. mc_task → mc_board
add_lookup mc_task mc_task_board mc_taskboardlookup "Board" mc_board "ApplicationRequired"
